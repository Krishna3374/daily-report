...
def aggregate_by(df, agg_type: str, period: str, is_comp: bool):
    ...
    if agg_type == 'total':
        ...
        modified_df = df.filter((f.trim(agg_type) != '') & f.isnotnull(agg_type))
...
def save_table(df, schema: str, table: str):
    if table.startswith('ly') or table.startswith('comp'):
        ...
        pre_action = f'''CREATE TABLE {schema}.{table} (report_date date, app_key bigint, profit_center varchar,
                        ...
                        financing_cost_pct decimal(10, 2), avg_ticket_amt decimal(10, 2),
                        items_per_ticket decimal(10, 2), avg_sales_price decimal(10, 2), close_rate_pct decimal(10, 2),
                        carts_built_pct decimal(10, 2), sales_per_guest decimal(10, 2), total_financed_amt decimal(10, 2),
                        financed_sales_pct decimal(10, 2), avg_financed_ticket_amt decimal(10, 2), finance_application_pct decimal(10, 2),
                        ...
                        avg_days_to_deliver decimal(10, 2), delivered_gross_margin_pct decimal(10, 2), bedding_sales_delivered decimal(10, 2)
                    );'''
    else:
        ...
        pre_action = f'''CREATE TABLE {schema}.{table} (report_date date, ly_date date, app_key bigint, profit_center varchar,
                        ...
                        online_sales_amt decimal(15, 2), orders_with_discount_pct decimal(10, 2), paid_hours decimal(15, 4),
...
logger.info(f"Job processing started: {job_start_time}")

logger.info('Getting all the required tables.')
...
daily_comp_prep = (report_base_df.alias('a').join(store_lookup_df.alias('b'), (f.col('a.app_key')==f.col('b.app_key'))
                    ...
                    .select('a.*', f.coalesce(f.col('grand_open_date'), '2020-01-01').cast('date').alias('store_open_date'))
                )
...
# ============= mtd section =============
logger.info('Calculating the MTD section TY values.')
...
logger.info('Calculating the MTD section LY values.')
...
logger.info('Calculating the MTD section Comp values.')
...
# ============= ytd section =============
logger.info('Calculating the YTD section TY values.')
...
logger.info('Calculating the YTD section LY values.')
...
logger.info('Calculating the YTD section Comp values.')
...
logger.info(f'Job completed: {job_end_time} | Run duration: {job_duration}')
